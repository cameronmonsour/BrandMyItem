...
test('dashboard keeps compact tracker cards without a duplicate left category box', () => {
  ...
  assert.match(html, /\.lcard \.iname\{[^}]*width:100%;height:19px;line-height:19px;[^}]*margin:14px 0 2px/);
...
test('sold and price pills keep the same compact height', () => {
  assert.match(html, /\.lcard \.iname\{display:block;width:100%;height:19px;line-height:19px;[^}]*margin:14px 0 2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis\}/);
