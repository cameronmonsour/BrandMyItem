...
test('removes the screenshot-only homepage and page headers', () => {
  ...
  assert.match(html, /how-it-works-monthly-check-in\.webp/);
  ...
  assert.match(html, /how-it-works-apply-brand-outline\.webp/);
...
test('dashboard keeps compact tracker cards without a duplicate left category box', () => {
  ...
  assert.match(builderHtml, /<span>Listing window<\/span><b>60 days, until funded or expired<\/b>/);
...
test('every catalog item has an item-specific branding method', () => {
  ...
  assert.match(methodsBlock, /macbook:'Sticker'/);
  assert.match(methodsBlock, /bottle:'Engraving'/);
  assert.match(methodsBlock, /golfbag:'Embroidery'/);
  assert.match(methodsBlock, /suit:'Patch'/);
  assert.match(methodsBlock, /paddle:'Sticker'/);
  assert.match(methodsBlock, /weekender:'Patch'/);
