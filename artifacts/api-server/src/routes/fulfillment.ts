...
router.post("/campaigns/:campaignId/checkins/photo/request-url", async (req, res): Promise<void> => {
  ...
  const campaign = await ownerCampaign(req, params.data.campaignId);
  if (!campaign) { res.status(404).json({ error: "Campaign not found" }); return; }
  if (!["due", "reminded"].includes(campaign.checkinStatus) || !campaign.checkinDueAt) { res.status(409).json({ error: "A check-in is not currently due." }); return; }
...
router.post("/campaigns/:campaignId/checkins/photo/:intentId/finalize", async (req, res): Promise<void> => {
  ...
  if (!["due", "reminded"].includes(campaign.checkinStatus)) { res.status(409).json({ error: "A check-in is not currently due." }); return; }
  const token = readAccessToken(req, "campaign", campaign.id)!;
  const [intent] = await db.select().from(uploadIntentsTable).where(eq(uploadIntentsTable.id, params.data.intentId)).limit(1);
  const now = new Date();
...
router.post("/campaigns/:campaignId/checkins", async (req, res): Promise<void> => {
  ...
  const submitted = await db.transaction(async (tx) => {
    ...
    const reserved = await tx.update(campaignsTable).set({ checkinStatus: "submitted", checkinDueAt: new Date(now.getTime() + 30 * 86400000), checkinReminderSentAt: null, updatedAt: now }).where(and(eq(campaignsTable.id, campaign.id), eq(campaignsTable.checkinStatus, campaign.checkinStatus))).returning({ id: campaignsTable.id });
...
router.post("/campaigns/:campaignId/make-good", async (req, res): Promise<void> => {
  ...
  if (!campaign) { res.status(404).json({ error: "Campaign not found" }); return; }
  const now = new Date();
  const updated = await db.update(campaignsTable).set({ makeGoodSelection: input.data.selection, makeGoodSelectedAt: now, updatedAt: now }).where(and(eq(campaignsTable.id, campaign.id), eq(campaignsTable.checkinStatus, "missed"))).returning({ id: campaignsTable.id });
